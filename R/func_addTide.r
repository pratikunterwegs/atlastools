...
#'
#' @param data A data frame which is the output of the clean data function \code{wat_clean_data}.
...
wat_add_tide <- function(data,
  ...
  if(!data.table::is.data.table(data)){
	...
	# include asserts checking for correct columns and data type
	{
      ...
      # check tide data
      # tide_data_names <- colnames(tide_data)
	...
	{
		temp_data <- data.table::merge.data.table(data, tide_data, by.x = "ts", by.y = "time",
                     all = TRUE)
