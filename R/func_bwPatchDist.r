...
#'
#' @param data A dataframe of or extending the class data.frame. This must contain two pairs of coordinates, the start and end X and Y coordinates of a feature.
...
#' @export
#'
wat_bw_patch_dist = function(data, x1 = "x_end", x2 = "x_start",
                             y1 = "y_end", y2 = "y_start"){
  ...
  dist <- dplyr::case_when(nrow(data) > 1 ~
                             ...
                             {
                               {
                                 x1 <- data[[x1]][1:nrow(data)-1]
                                 x2 <- data[[x2]][2:nrow(data)]
                               }
                               {
                                 y1 <- data[[y1]][1:nrow(data)-1]
