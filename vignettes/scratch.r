...
`atlastools` has the function `atl_get_data` Gets data from the NIOZ server.
`atl_get_data` is untested because the data are stored on the NIOZ servers. I haven't worked out whether connectting and retrieving this data is actually possible. An example is provided, but will not work, because the data access arguments are not public.

```{r get_raw_data}
# get an example raw data file

prelim_data <- atl_get_data(tag = 435,
...
temp_data <- atl_rm_attractor(df = raw_data,
                              atp_xmin = atp$xmin,
                              atp_xmax = atp$xmax,
                              atp$ymin = atp$ymin,
                              atp$ymax = atp$ymax)
...
clean_data <- atl_clean_data(somedata = temp_data,
                             moving_window = 3,
                             nbs_min = 0,
                             sd_threshold = 100,
                             filter_speed = TRUE,
...
For data quality and computational reasons, we might want to aggregate the data over a specific interval. Here, we use a 30 second interval for aggregation. This is done using `atl_agg_data`.

```{r clean_raw_data}
# aggregate over 30 seconds
agg_data <- atl_agg_data(df = clean_data, interval = 30)
```
...
This means adding the time of high tide to the data using `atl_add_tide`,
The function needs a data frame of high tide times; this is not covered here.

```{r add_tide_data}
# add tide data
agg_data <- atl_add_tide(df = agg_data,
                         tide_data = "../private/")
```